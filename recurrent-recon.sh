...
grep -F -x -v -f $DIR/alive.old $DIR/alive.txt | tee -a ~/output-$TODAY.txt
rm $DIR/alive.old
...
### Display new ports ### 
echo "New ports: " >> ~/output-$TODAY.txt
grep -Fxvf $DIR/masscan.old $DIR/masscan.txt | tee -a ~/output-$TODAY.txt
